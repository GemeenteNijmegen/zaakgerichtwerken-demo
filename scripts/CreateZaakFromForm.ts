...
export async function createZaakFromForm() {
  ...
  const response = await fetch(ZAKEN_API + 'zaken', {
    method: 'POST',
    body: JSON.stringify({
      identificatie: messageJson.reference,
